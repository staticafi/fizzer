...
//                                         generated_path
// ------------------------------------------------------------------------------------------------
bool generated_path::contains( location_id id ) const { return path.contains( id ); }

// ------------------------------------------------------------------------------------------------
std::map< location_id, node_props_in_path > generated_path::get_path() const { return path; }
...
// ------------------------------------------------------------------------------------------------
const std::unordered_set< location_id >& fuzzing::equation_matrix::get_node_ids() const
{
    return node_ids;
}
...
//                                    loop_properties
// ------------------------------------------------------------------------------------------------
bool fuzzing::loop_properties::is_same( const std::unordered_set< location_id >& other_ids ) const
{
    TMPROF_BLOCK();
...
// ------------------------------------------------------------------------------------------------
const std::unordered_set< location_id >& fuzzing::loop_properties::get_all_ids() const
{
    TMPROF_BLOCK();
    return all_ids;
}
...
// ------------------------------------------------------------------------------------------------
const std::unordered_set< location_id >& fuzzing::loop_properties::get_loop_head_ids() const
{
    TMPROF_BLOCK();
...
// ------------------------------------------------------------------------------------------------
location_id fuzzing::loop_properties::get_smallest_loop_head_id() const
{
    TMPROF_BLOCK();

    location_id smallest_id = *loop_head_ids.begin();
...
// ------------------------------------------------------------------------------------------------
std::optional< location_id > fuzzing::loop_properties::get_smallest_body_id() const
{
    TMPROF_BLOCK();

    std::optional< location_id > smallest_id;
...
// ------------------------------------------------------------------------------------------------
location_id fuzzing::loop_properties::get_smallest_id() const
{
    TMPROF_BLOCK();
    location_id smallest_id = *all_ids.begin();
...
//                                     loop_dependencies
// ------------------------------------------------------------------------------------------------
loop_properties& fuzzing::loop_dependencies::get_props( const std::unordered_set< location_id >& ids,
...
void fuzzing::loop_dependencies::merge_properties()
{
    ...
    std::unordered_map< location_id, loop_properties > merged_loops;

    for ( const auto& loop : loops ) {
        location_id smallest_id = loop.get_smallest_id();
    ...
    for ( auto& [ id, loop ] : merged_loops ) {
        const std::unordered_set< location_id >& head_ids = loop.get_loop_head_ids();
...
// ------------------------------------------------------------------------------------------------
const loop_properties& fuzzing::loop_dependencies::get_props_by_loop_head_id( location_id loop_head_id ) const
{
...
// ------------------------------------------------------------------------------------------------
loop_properties& fuzzing::loop_dependencies::get_props_by_loop_head_id( location_id loop_head_id )
{
...
// ------------------------------------------------------------------------------------------------
std::set< location_id > fuzzing::loop_dependencies::get_loop_heads( bool include_loading_loops ) const
{
                                             ...
                                             std::size_t max_directions_in_path_index )
{
    ...
    auto it = branching_values.find( end_node->get_best_value() );
                                                     ...
                                                     const iid_node_generations_stats& stats )
{
    ...
    bool is_equal_sign = get_branching_predicate() == atomic_predicate::EQUAL;
...
void equation_matrix::print_matrix()
{
    ...
    for ( std::size_t i = 0; i < matrix.size(); ++i ) {
        for ( std::size_t j = 0; j < matrix[ i ].values.size(); ++j ) {
...
// ------------------------------------------------------------------------------------------------
atomic_predicate equation_matrix::get_branching_predicate() const
{
    ASSUMPTION( all_paths.size() > 0 );
    return all_paths[ 0 ]->get_atomic_predicate();
}
...
void fuzzing::equation_matrix::add_path( branching_node* end_node,
                                         ...
                                         std::size_t max_directions_in_path_index )
{
    auto branching_values_it = branching_values.find( end_node->get_best_value() );
    ...
    if ( add_columns ) {
        ...
        for ( std::size_t i = 0; i < max_directions_in_path_index + 1; ++i ) {
            ...
            for ( bool direction : { true, false } ) {
                nodes.push_back( { location_id( id ), direction } );
                added_nodes++;
            }
        }
    ...
    equation row = { values_in_path, end_node->get_best_value() };

    auto [ it, inserted ] = unique_rows.insert( row );
    if ( inserted ) {
        matrix.push_back( row );
        branching_values[ end_node->get_best_value() ]++;
    }
}
...
generated_path iid_node_dependence_props::generate_probabilities( const loop_dependencies& loop_to_properties )
{
    ...
    std::unordered_set< location_id > subset_ids;
    ...
    if ( iid_dependencies::verbose ) {
        ...
        for (std::size_t i = 0; i < new_subset_counts->values.size(); ++i) {
...
{
    return matrix.get_branching_predicate() == atomic_predicate::EQUAL;
}
                                                ...
                                                const std::unordered_set< location_id >& subset_ids )
{
    ...
    std::set< location_id > loop_heads = loop_to_properties.get_loop_heads( false );
...
//                                 iid_dependencies
// ------------------------------------------------------------------------------------------------
void iid_dependencies::update_ignored_nodes( std::unordered_set<fuzzing::branching_node *> const& changed_nodes )
{
    for ( branching_node* node : changed_nodes ) {
        if ( node->is_did_branching() ) {
            location_id location_id = node->get_location_id();
...
iid_vector_analysis_statistics fuzzing::iid_dependencies::get_stats() const
{
    ...
    for ( const auto& [ id, props ] : node_id_to_equation_map ) {
        ...
        node_stats.node_ids = std::vector< location_id >( props.get_matrix().get_node_ids().begin(),
    ...
    stats.ignored_node_ids = std::vector< location_id >( ignored_node_ids.begin(),
                                                               ...
                                                               loop_head_to_bodies_t& loop_heads_to_bodies )
{
    ...
    auto is_outside_loop = [ & ]( branching_node* successor,
        ...
        return !loop_bodies.contains( successor->get_location_id() );
    };
    ...
    for ( const auto& loop : loops ) {
        ...
        if ( is_outside_loop( left_successor, loop_head_id, loop_bodies ) ) {
            loop_heads_ending[ loop.exit->get_location_id() ] = false;
        } else if ( is_outside_loop( right_successor, loop_head_id, loop_bodies ) ) {
            loop_heads_ending[ loop.exit->get_location_id() ] = true;
        }
    }
                                                                 ...
                                                                 const loop_endings& loop_heads_ending )
{
    ...
    struct dependent_body_props {
        ...
        std::unordered_set< natural_32_bit > sensitivity_bits = {};
    };

    std::unordered_map< location_id, std::unordered_map< location_id, dependent_body_props > > loop_head_to_props;
    std::unordered_map< location_id, dependent_body_props > node_id_to_props;

    branching_node* node = end_node;

    while ( node != nullptr ) {
        const auto node_id = node->get_location_id();
        const auto& sensitive_bits = node->get_sensitive_stdin_bits();
                                                               ...
                                                               const loop_endings& loop_heads_ending )
{
    ...
    for ( const auto& [ loop_head_id, loop_bodies ] : loop_heads_to_bodies ) {
        ...
        const std::unordered_set< location_id >& all_props_ids = props.get_all_ids();

        for ( const auto& body : loop_bodies ) {
            if ( all_props_ids.contains( body ) ) {
                continue;
            }

            for ( bool direction : { true, false } ) {
                auto [ it, inserted ] = props.bodies.emplace( body, direction );
                changed |= inserted;
            }
        }
    }
                                                       ...
                                                       const loop_endings& loop_heads_ending )
{
    ...
    while ( node != nullptr ) {
        ...
        if ( auto it = loop_heads_to_bodies.find( node_id ); it != loop_heads_to_bodies.end() ) {
            auto& props = loading_loops[ node_id ];
        ...
        if ( node->get_predecessor() != nullptr ) {
            branching_node* predecessor = node->get_predecessor();
            const auto predecessor_id = predecessor->get_location_id();
...
void fuzzing::iid_dependencies::compute_paths( branching_node* end_node )
{
    ...
    while ( current_node->get_predecessor() != nullptr ) {
...
std::vector< node_id_with_direction > fuzzing::iid_dependencies::get_path( branching_node* end_node )
{
    ...
    while ( current != nullptr ) {
        branching_node* predecessor = current->get_predecessor();
        iid_seen = iid_seen || is_tracked( current->get_location_id() );

        if ( iid_seen && predecessor != nullptr ) {
            node_id_with_direction nav = { predecessor->get_location_id(),
...
bool fuzzing::iid_dependencies::is_tracked( location_id id ) const
{
    ...
    return !ignored_node_ids.contains( id ) && !covered_node_ids.contains( id );
}
...
std::pair< path_id_direction_count, std::size_t > get_directions_in_path( branching_node* node )
{
    ...
    while ( current != nullptr ) {
        ...
        if ( predecessor != nullptr ) {
            int id = predecessor->get_location_id();
